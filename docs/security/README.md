...
**Fix (migration owner-locked):** un gate solo, nel seam comune: in `promotion_target` dopo il check "proposta aperta" aggiungere `if not public.is_project_member(c.proposal_id → project) then raise exception 'non membro del progetto'` (via `public.proposal_project(c.proposal_id)` o join diretta, la funzione è definer). pgTAP: outsider autore → `throws_ok 'non membro del progetto'` su request e revoke. **Stato:** implementato in `0022` [4] + `rls_projects_test.sql`, in attesa di `db push`.
