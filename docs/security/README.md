...
- **Server Action authorization** — mutations are Server Actions. Each one must re-check the authenticated user and role server-side; never trust client-passed identity or role. Since migration 0020 the scan/eval outcome RPCs and `auth.admin` calls run as `service_role` (no RLS backstop): the guard in the calling Server Action is the *only* gate, so every new caller of `runProposalScan`, `runEvaluation` or `supabaseAdmin()` must authorize (user + role/ownership + status) before the call.
...
**Stato 2026-09-05:** la parte residua (forging dentro il range) è chiusa dalla migration `0020_admin_powers_service_role.sql` insieme a SEC-9 — `begin/apply/fail_ai_evaluation` eseguibili solo da `service_role`, `can_run_ai_evaluation` eliminata, scritture via `supabaseAdmin()` in [`src/lib/ai/runEvaluation.ts`](../../src/lib/ai/runEvaluation.ts). Applicata in locale; **resolved solo dopo l'apply sul remoto** (vedi SEC-9).
...
**Remediation (decisa 2026-07-09, implementata 2026-09-05 — branch `newFeatures`, in attesa di apply):** il soggetto vincolato dal gate non scrive più l'esito del gate. Migration [`0020_admin_powers_service_role.sql`](../../supabase/migrations/0020_admin_powers_service_role.sql): `revoke execute` su `begin/apply/fail_dup_scan` e `begin/apply/fail_ai_evaluation` da `public, anon, authenticated`, `grant execute` a `service_role`, `drop` di `can_run_dup_scan`/`can_run_ai_evaluation`, DML su `public.*` a `service_role`. Lato app [`src/lib/supabase/admin.ts`](../../src/lib/supabase/admin.ts) (`SUPABASE_SERVICE_ROLE_KEY`, server-only) e scritture via `supabaseAdmin()` in [`src/lib/ai/runProposalScan.ts`](../../src/lib/ai/runProposalScan.ts) / [`runEvaluation.ts`](../../src/lib/ai/runEvaluation.ts); le letture restano col client utente (RLS) e `runProposalScan` continua a rifiutare status ≠ `nuova`. Autorizzazione ora SOLO nei guard delle Server Action chiamanti (`runProposalScanAction`, `evaluateProposal`, `updateProposal`, `editComment`, `resolve/revokeCommentPromotion`) — nessun backstop DB (vedi `2026-07-03-adm1` in [`be-careful.md`](be-careful.md)). Verificato da `supabase/tests/rls_admin_powers_test.sql` (pgTAP) e `tests/integration/promotion-and-votes.test.ts`.

**Per chiudere (owner):** (1) ADR che sana il pattern RPC user-called di ADR-0005; (2) `supabase db push` della 0020 sul remoto; (3) `SUPABASE_SERVICE_ROLE_KEY` nell'env di produzione (senza, scan/eval/inviti falliscono chiusi: `supabaseAdmin()` lancia); (4) replicare a mano il template invite sul remoto (Dashboard → Auth → Email Templates). Poi marcare SEC-9 e SEC-8 `✅ resolved`.

### SEC-10 — Utente disabilitato: access token valido su PostgREST fino a `exp` (LOW)

**Where:** [`src/app/profile/actions.ts`](../../src/app/profile/actions.ts) `setUserDisabled` (`auth.admin.updateUserById(id, { ban_duration: "876600h" | "none" })`), `supabase/config.toml` (`jwt_expiry = 3600`), [`src/proxy.ts`](../../src/proxy.ts) (`getUser()` a ogni request).

**Issue:** "Disabilita" è il controllo di offboarding, ma agisce solo su GoTrue: il ban blocca login e refresh token, e il proxy/le Server Action (che chiamano `getUser()` verso GoTrue) perdono la sessione al più tardi al primo refresh. PostgREST però verifica il JWT localmente e non conosce il ban: l'access token già emesso resta accettato fino a `exp`, cioè fino a 1 ora, per query dirette con l'anon key e RLS da `authenticated`.

**Impact:** Low. Finestra limitata a `jwt_expiry`, solo ciò che RLS già permette al ruolo `authenticated` (lettura di tutto, scritture self-scoped), nessuna escalation. Rilevante solo per un offboarding ostile.

**Fix when touched:** ridurre `jwt_expiry` (es. 600–900 s; il refresh via cookie mantiene l'UX) — il ban blocca il refresh e la finestra si riduce di conseguenza. In alternativa, all'atto del ban, invalidare anche le sessioni (`delete from auth.sessions where user_id = …` via RPC `security definer` eseguibile solo da `service_role`): chiude il refresh subito, ma l'access token in corso resta comunque valido fino a `exp`.
