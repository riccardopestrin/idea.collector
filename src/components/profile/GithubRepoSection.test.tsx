...
describe("GithubRepoSection", () => {
  ...
  it("offers only the connect button when GitHub is not connected", async () => {
    ...
    render(<GithubRepoSection connected={false} selectedRepo={null} repos={[]} />);
    ...
    expect(startGithubConnect).toHaveBeenCalled();
  });

  it("lists the authorized repos with a placeholder until one is chosen, and submits the choice", async () => {
    const user = userEvent.setup();
    render(<GithubRepoSection connected selectedRepo={null} repos={repos} />);
    ...
    expect(vi.mocked(selectRepo).mock.calls[0][1].get("repo")).toBe("acme/site");
  });

  it("shows the connected repo preselected without the placeholder, and can disconnect", async () => {
    const user = userEvent.setup();
    render(<GithubRepoSection connected selectedRepo="acme/ideas" repos={repos} />);
